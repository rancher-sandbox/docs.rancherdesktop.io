...
The descriptions are not baked into `explain`; they come straight from
...
the next start picks up from wherever the world actually is. A script would
have assumed it already knew.

## Why build it this way

Because it's the ordinary Kubernetes API, the standard tools already work on
it. There's `explain` to read it, `--dry-run` and `--wait` to script it,[^wait]
and `rdd ctl get app -w` to watch it change.

That also makes Rancher Desktop easy to automate, from a shell script
