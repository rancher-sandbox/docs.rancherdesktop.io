...
server, and it holds Rancher Desktop's own state as objects you can query. So
there are actually two Kubernetes API servers on your machine, and only one of
them runs your pods.
...
and said it deserved a post of its own, so this is that post.

That second API server is up before you enable Kubernetes, and it stays up
after you turn Kubernetes off again. That sounds odd, but it gets simpler once
you know that Kubernetes really is two separate things sharing a single name.
...
conditions reporting how far along it got. You only ever edit the `spec`, and
the controllers write the `status`. Every Kubernetes resource works this way,
...
missing, so there are no pods and no nodes. The server has never even heard of
a node:
...
work, because there was never anything custom to support.
...
[^singleton]: There is only ever one, it's cluster-scoped, and it must be named `app`. Hence `get app app`, the kind followed by the name.
